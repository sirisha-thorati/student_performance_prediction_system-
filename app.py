...
def login():
    ...
    cursor.execute(
        "SELECT register_number FROM users WHERE username=%s AND password=%s",
        (username, password)
    )
...
def dashboard():
    ...
    # Get student name
    cursor.execute(
        "SELECT name FROM users WHERE register_number=%s",
        (reg_no,)
    )
    user = cursor.fetchone()
    ...
    # Get subject + marks
    cursor.execute(
        "SELECT subject, marks FROM marks WHERE register_number=%s",
        (reg_no,)
    )
...
# Flask Connection(Register Page)
...
def marks():
    ...
    cursor.execute("""
        SELECT subject_name 
        FROM subjects
        WHERE department=%s AND year=%s AND semister=%s
    """,(dept,year,sem))
...
def predict():
    ...
    cursor.execute(
        "DELETE FROM marks WHERE register_number=%s",
        (reg_no,)
    )
    ...
    cursor.executemany(
        "INSERT INTO marks (register_number, subject, marks) VALUES (%s, %s, %s)",
        marks_data
    )
...
def insert_subjects():
    ...
    cursor.executemany(
        "INSERT INTO subjects (subject_name, department, year, semister) VALUES (%s, %s, %s, %s)",
        subjects_data
    )
